...
const AdminHotel = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      dispatch({
        type: "fetchHotels",
        payload: { hotels: response_hotels.data },
      });
    }
    fetchData();
  }, []);
  ...
  const fileUploadHandler = () => {
    ...
    axios.post("http://localhost:3000/hotel/", formData, config).then((res) => {
  ...
  const adminDeleteHotel = (hotel_id: number) => {
            ...
            <Card variant="outlined">
              ...
              <CardActions>
                <Button
                  ...
                  style={{ color: "white", fontWeight: "bold" }}
                >
                  <Link to={"/admin/room/" + hotel.id}>Show Rooms</Link>
                </Button>
                <Button
                  size="small"
                  onClick={() => {
                    adminDeleteHotel(hotel.id);
                  }}
                  style={{
